...
const newArrivalIds = [
  'osah-edge-pro-40l-duffel-bag',
  'mh02-traillock-x-pro-phone-camera-mount',
  'kp16ep16-mx-knee-elbow-guard',
  'mjw-6l-crash-bar-bag'
];
...
function renderNewArrivals() {
  ...
  if (brands) brands.textContent = String(new Set(products.map(product => product.brand).filter(name => name !== 'UNBRANDED')).size);
}
...
fetch('data/products.json').then(response => response.json()).then(data => {
  for (const item of data) if (!CATEGORIES.includes(item.category)) CATEGORIES.push(item.category);
