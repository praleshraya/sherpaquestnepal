...
from product_content import story, feature_rows, options_for, color_preview, SWATCHES
...
for product in PRODUCTS:
    ...
    highlights = ''.join(f'<li><strong>{esc(label)}:</strong> {esc(value)}</li>' for label, value in facts if label not in ('Brand', 'Gear type')) or f'<li>{product_story}</li>'
    ...
    gallery_controls = f'''<div class="product-thumbs" aria-label="Product images">{''.join(f'<button type="button" class="product-thumb{ " active" if index == 0 else "" }" data-gallery-src="../{esc(item["src"])}" data-gallery-alt="{esc(product["name"])} — {esc(item["variant"])}" data-gallery-color="{esc(gallery_color(product, item["src"], colors))}" aria-label="Show {esc(item["variant"])} image {index + 1}" aria-pressed="{str(index == 0).lower()}"><img src="../{esc(item["src"])}" alt="" loading="lazy"></button>' for index, item in enumerate(gallery))}</div>''' if len(gallery) > 1 else ''
...
<link rel="stylesheet" href="../styles.css?v=3"><link rel="stylesheet" href="../product.css?v=3"><link rel="stylesheet" href="../theme.css?v=2"><script defer src="../navigation.js?v=3"></script><script defer src="../product-gallery.js?v=2"></script><script defer src="../floating-promo.js"></script></head>
...
<div class="product-layout"><div class="product-gallery"><div class="product-main-image"><img id="product-main-image" src="../{esc(product['image'])}" data-primary-src="../{esc(product['image'])}" alt="{esc(product['name'])}"></div>{gallery_controls}</div>
<div class="product-info"><p class="eyebrow"><a href="../index.html?brand={quote(product['brand'])}#products">{esc(product['brand'])}</a> / {esc(product['category'])}</p><h1>{esc(product['name'])}</h1>
